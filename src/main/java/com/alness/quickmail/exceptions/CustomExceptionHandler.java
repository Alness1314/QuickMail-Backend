...
public class CustomExceptionHandler {
    ...
    public ResponseEntity<Map<String, String>> handleValidationExceptions(MethodArgumentNotValidException ex) {
        ...
        errors.put("code", CodeUtils.API_CODE_400);
    ...
    public ResponseEntity<ErrorResponse> handleNotFoundError(NoHandlerFoundException ex) {
        ...
        ErrorResponse error = ErrorResponse.builder().code(CodeUtils.API_CODE_404).message("No se encontró el recurso solicitado, "+ endpoint).build();
        return new ResponseEntity<>(error, HttpStatus.NOT_FOUND);
    }

    @ExceptionHandler(RuntimeException.class)
    public ResponseEntity<ErrorResponse> runtimeExceptionHandler(RuntimeException ex) {
        ErrorResponse error = ErrorResponse.builder().code(CodeUtils.API_CODE_500).message(ex.getMessage()).build();
